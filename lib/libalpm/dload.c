		...
		const char *localpath)
{
	...
	if(pid == 0) {
		...
		_alpm_log(handle, ALPM_LOG_WARNING, _("passing %d to parent!\n"), ret);
	...
	if(pid != -1)  {
		...
		if(wret > 0) {
			...
			_alpm_log(handle, ALPM_LOG_WARNING, _("got return code of %d!\n"), ret);
		}
	} else {
	...
	_alpm_log(handle, ALPM_LOG_WARNING, _("returning %d!\n"), ret);
	return ret;
}
		...
		const char *localpath)
{
	if(handle->fetchcb == NULL) {
		_alpm_log(handle, ALPM_LOG_WARNING, _("no external command\n"));
#ifdef HAVE_LIBCURL
		if(handle->usesandbox) {
			_alpm_log(handle, ALPM_LOG_WARNING, _("sandbox enabled\n"));
			return curl_download_internal_sandboxed(handle, payloads, localpath);
		} else {
			_alpm_log(handle, ALPM_LOG_WARNING, _("sandbox disabled\n"));
